...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const response = await fetch(`${MEDUSA_API_URL}/store/shipping-options?cart_id=${cartId}`, {
      headers: {
    ...
    // Override shipping prices to 0€ for now
    if (data.shipping_options && Array.isArray(data.shipping_options)) {
      data.shipping_options.forEach((opt: any) => {
        if (opt.calculated_price) {
    ...
    return new Response(JSON.stringify({ error: 'Failed to get shipping options' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
